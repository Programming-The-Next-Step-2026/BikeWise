...
# NOTE: precipitation is forecast at 15-minute resolution.
# Suggested departure time is rounded to nearest 15 minutes.
# Limitation in UI?
...
#'   Defaults to 1 km.
#' @param max_rain_level How much rain is too much to cycle in. One of
...
#'   minutes count as near the end. Defaults to 2.
#'
...
raintracker <- function(timed_df,
                        start_time,
                        check_interval_km = 1,
                        max_rain_level = "moderate",
                        end_buffer_min = 2) {

  # Parse start_time to POSIXct if supplied as a string
  if (!inherits(start_time, "POSIXct")) {
    start_time <- as.POSIXct(start_time, tz = "Europe/Amsterdam")
  }
  ...
  max_dist  <- max(timed_df$dist_km)
  multiples <- seq(0, max_dist, by = check_interval_km)
  idx <- sapply(multiples, function(m) which.min(abs(timed_df$dist_km - m)))
  idx <- sort(unique(c(idx, which.max(timed_df$dist_km))))
  ...
  repeat {
    ...
    for (i in seq_len(nrow(checkpoints))) {
      ...
      if (exceeds_threshold(level, max_rain_level)) {
        ...
        if (is_end_buf) {
          ...
          safe_rows <- future[!sapply(
            future$mm_h,
            function(x) exceeds_threshold(classify_rain(x), max_rain_level)
          ), ]
