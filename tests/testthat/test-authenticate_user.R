...
test_that("authenticate_user creates new user with NA tolerance and speed", {
  written <- NULL
  local_mocked_bindings(
    sheet_id    = function() "dummy",
    read_sheet  = function(...) fake_users,
  ...
  authenticate_user("bob", "mypassword")
  new_row <- written[written$username == "bob", ]
  expect_true(is.na(new_row$rain_tolerance))
  expect_true(is.na(new_row$cycling_speed))
})
...
test_that("authenticate_user creates new user with NA defaults (example)", {
  tmp <- withr::local_tempdir()
  local_mocked_bindings(
    R_user_dir = function(...) tmp,
    .package = "BikeWise"
